...
| **Duress passwords** | Up to three additional passwords per account (`POST /api/v1/personas`), each unlocking a fully separate identity with its own bucket grants and an optional flag that revokes every other live session on login - deniable under coercion, with no silent alarm. | [docs/operations.md#duress-personas](docs/operations.md#duress-personas) |
