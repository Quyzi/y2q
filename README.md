...
| **Identity secret keys at rest** | Every account carries four fixed credential slots (real or decoy, byte-shape identical). Each slot's ML-KEM-768 identity secret key is Argon2id-wrapped under its own password; once unwrapped on login it is held for the session's whole lifetime (bounded by `auth.max_ttl_seconds`) as AES-256-GCM ciphertext under a process-ephemeral, guarded-memory key (Linux) - plaintext exists only transiently, inside a locked buffer, for the duration of a single operation. There is no idle-drop knob, because there is no process-wide key left to drop. | [crypto/kdf.rs](crates/y2q-core/src/crypto/kdf.rs), [auth/session.rs](crates/y2qd/src/auth/session.rs), [secmem.rs](crates/y2q-core/src/secmem.rs) |
| **Bucket secret keys at rest** | Each bucket has its own ML-KEM-768 keypair per retained key epoch; its secret is wrapped under a bucket wrap key sealed individually to every authorized grantee's identity - not a single deployment-wide key. A session's resolved bucket keys are cached the same way as its identity key: sealed, not plaintext. Global `admin`/`auditor` roles hold no bucket key material at all: visibility follows the crypto grant, not the role. | [crypto/seal.rs](crates/y2q-core/src/crypto/seal.rs) |
...
\* Guarded memory is **Linux-only**. On macOS/Windows, `secmem` falls back to a `Zeroizing`-backed scheme with none of the above - plaintext sits in ordinary, swappable, dumpable heap the whole session. See [SECURITY.md](SECURITY.md#guarded-memory-linux-only).

**What it does not defend against:** a live memory read timed to a request actually in flight (guarded memory closes *idle* disclosure, not a read mid-operation), object body plaintext in ordinary heap, a leaked Bearer token until it expires or is revoked, and traffic analysis when TLS is disabled. See the full [threat model](docs/architecture.md#threat-model-brief) and [SECURITY.md](SECURITY.md) for the complete boundary.
