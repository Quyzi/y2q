...
On login, one Argon2id derivation (against the record's shared salt) yields a KEK; the daemon tries all four slots' unwrap *without short-circuiting*, so response timing never reveals which slot — real or decoy — actually opened, nor how many of the four are live. The recovered identity secret key is immediately sealed as AES-256-GCM ciphertext under a process-ephemeral, guarded-memory wrapping key (Linux; see [Guarded memory](../SECURITY.md#guarded-memory-linux-only)) and stored that way in that session's `SessionInfo` (`crates/y2qd/src/auth/session.rs`) for the session's whole lifetime — plaintext exists only transiently, inside a locked buffer, for the duration of one crypto operation (`with_identity_sk`). There is no process-wide keystore slot analogous to the old shared MEK, and nothing to idle-drop.
...
In-memory `DashMap<[u8; 32], Arc<SessionInfo>>`. Each `SessionInfo` carries the username, global role, timestamps, this persona's slot index, its identity secret key (never plaintext at rest — sealed as `SealedSecret` ciphertext under a per-store `SessionKeyring`, see [Session-scoped identity keys](#session-scoped-identity-keys) below), and a bounded per-session bucket-key cache (also sealed, not plaintext). There is no persistence: a daemon restart invalidates every session.
...
- **Global role** - an account-wide capability ceiling stored on the `UserRecord`: `admin` (full verb ceiling on buckets this persona can decrypt, plus admin endpoints), `user` (governed by bucket grants), `readonly`/`writeonly` (read-or-write only on owned/granted buckets), `auditor` (read-only on buckets this persona can decrypt, plus read-only admin endpoints), `disabled` (nothing). A global role is not a visibility bypass.
- **Per-bucket ownership + ACL** - each bucket has an owner (full control) and an optional grant map (`read`/`write`/`writeonly`/`admin`). New buckets are private to the persona that created them. A bucket this persona has no sealed grant for is hidden: it is omitted from listings and any direct operation returns 404 (never 403), so existence cannot be probed. 403 appears only on a bucket you can already see but lack the verb for. The exception is an ACL relationship of exactly `writeonly`: write needs no sealed secret, so write stays allowed and a read is 403.
- **Cryptographic bucket-key grant** - listing, HEAD metadata, reading, writing, deleting, and ACL changes all require the caller's *persona* to hold a real, sealed bucket-key grant (`bucket_keys::is_visible` / `resolve_read_key`). A global `admin`/`auditor` role does not skip that check, so `GET /` does not list bucket names the persona cannot decrypt, and an ungranted GET/HEAD/PUT/DELETE/ACL is **404**, not 403. An admin who personally holds a real grant still sees and reads that bucket. Creating a brand-new bucket still works for any role that can write. There is no admin group key, no escrow, and no break-glass path.

The effective capability for an action is the intersection of the role ceiling and the bucket relationship, and it is empty unless this persona holds a sealed grant (or the relationship is exactly `writeonly`, which keeps write only). With `enforce_authorization = false`, the ACL/role layers are skipped (single-user / migration mode) and the old short-circuit stays - every authenticated caller has full access. Object plaintext is still sealed under the bucket key. Full model and status codes: [api.md](api.md#authorization).

### Duress personas

Every `UserRecord` carries four credential slots (see [Key hierarchy](#key-hierarchy-and-identity-protection-at-rest)); each account's real identity is placed at a slot chosen uniformly at random on creation - there is no privileged slot number a caller or a coercer can rely on - and the other three are self-service alternates a user can populate via `POST /api/v1/personas` (`y2q persona add`). Each persona is a fully separate identity with its own bucket grants - there is no shared-access, silent-alarm design. A persona created with `revoke_other_sessions: true` silently switches every other live session of the account over to itself on login, in place - same tokens, same expiry, no revocation and nothing observably interrupted, just narrower access from that point on. This is the only side effect of a duress login, and it is not observable as one: no alert, no log line, and no metric distinguishes it from an ordinary one (`y2q_auth_logins_total{result}` never gains a duress label; `GET /api/v1/personas/me` never reports the duress flag, even for the caller's own session). A bucket the duress persona wasn't granted is 404, not 403, on read, write, delete, HEAD, and ACL - identical to any bucket that genuinely doesn't exist, and the same for an `admin` duress persona. So a 403 can never be used to confirm a real bucket's existence and betray the primary password. (A `writeonly` drop box is the exception: that caller can write, and a read is 403, because they were given the name.) `POST /api/v1/personas` and `DELETE /api/v1/personas/{slot}` apply to every slot except the caller's own active one, including `primary_slot`, and the response does not depend on which slot it was. A silent skip would let the next login fail only for the real slot and identify it; with only four slots, redirecting the write onto another one collides and is itself detectable. `primary_slot` is still the slot third parties grant to, not a slot the account's other personas are forbidden to replace. Granting from a third party reaches only the grantee's real identity (`UserRecord::primary_slot`, resolved server-side, never returned by any API); sharing access with one of your own alternate personas is self-service (`POST /api/v1/personas/{slot}/grant`), because a third party granting a *named* alternate persona would first have to know it exists.
...
- **A compromised administrator account** - this is the central property the per-bucket key hierarchy exists for. `admin`/`auditor` are cryptographically excluded from buckets they hold no sealed grant for: the role does not list those names, and GET/HEAD/PUT/DELETE/ACL return 404 rather than 403. An admin who personally holds a real grant still sees and reads that bucket. There is no admin group key, no escrow secret, and no break-glass self-grant - an admin can only read what they hold a real grant for, exactly like any other user.
...
- **Memory disclosure while idle (Linux only)** - core dumps and swap do not contain session identity keys, bucket keys, the process `MemoryKey`, node-derived keys, passwords, or bearer tokens. Those values are held in `memfd_secret` pages (`SecretBuf`/`SecretVec`, directly or as AES-256-GCM ciphertext under the `MemoryKey`, which is itself one of those pages). The pages are locked, excluded from core dumps, and not readable via `ptrace` or `/proc/<pid>/mem`, including while unlocked. The daemon additionally sets `PR_SET_DUMPABLE=0` and `RLIMIT_CORE=0` at boot and refuses to start if `memfd_secret` is unavailable, unless `[server] allow_unprotected_memory` is set. See [SECURITY.md](../SECURITY.md#guarded-memory-linux-only) for the full mechanism and its **Linux-only** scope - on macOS/Windows this bullet does not apply; see [Platform support](#platform-support) below.

What it doesn't defend against:

- **Compromised running daemon, request in flight** - object-body plaintext still lives in ordinary heap, and a root reader can still recover a request's body while it is in flight. `memfd_secret` pages (identity keys, bucket keys, the process `MemoryKey`, node-derived keys, passwords, bearer tokens held in `SecretBuf`/`SecretVec`) are not readable via `ptrace` or `/proc/<pid>/mem`, including while unlocked. If `memfd_secret` is unavailable the daemon refuses to start unless `allow_unprotected_memory` is set. Session TTL (`[auth] max_ttl_seconds`) bounds how long a leaked identity key would stay valid if it were obtained some other way (kernel compromise, physical RAM).
...
**Guarded memory is a Linux-specific hardening layer, not a portability concern in the same sense.** `y2q_core::secmem`'s `mmap`/`mprotect`/`mlock`/`madvise` guard-paged allocations and `harden_process()`'s `PR_SET_DUMPABLE=0`/`RLIMIT_CORE=0` are gated `#[cfg(target_os = "linux")]`; other targets get a `Zeroizing`-backed fallback behind the identical API (so everything still compiles and runs), but with none of the actual protection - plaintext sits in ordinary, swappable, dumpable, debuggable heap. See [SECURITY.md](../SECURITY.md#guarded-memory-linux-only) for the full boundary.
...
- [crates/y2q-core/src/secmem.rs](../crates/y2q-core/src/secmem.rs) - guarded memory (`SecretBuf`/`SecretVec`/`MemoryKey`/`SecretString`), process hardening, Linux vs. portable-fallback split
- [crates/y2qd/src/auth/session.rs](../crates/y2qd/src/auth/session.rs) - session store, token hashing, per-persona `SessionInfo`
- [crates/y2qd/src/auth/keyring.rs](../crates/y2qd/src/auth/keyring.rs) - `SessionKeyring`, the process-ephemeral wrapping key sessions seal identity/bucket keys under
