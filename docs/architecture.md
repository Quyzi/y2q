...
- **Memory disclosure while idle (Linux only)** - a core dump, swap/hibernation image, same-uid `ptrace`/`/proc/<pid>/mem` read, or a VM snapshot taken while no request is mid-flight recovers no session identity keys, bucket keys, passwords, or bearer tokens. They are held as AES-256-GCM ciphertext (`SealedSecret`) under a process-ephemeral key living in a locked, non-dumpable, `PROT_NONE`-at-rest page (`y2q_core::secmem`); the daemon additionally sets `PR_SET_DUMPABLE=0` and `RLIMIT_CORE=0` at boot and refuses to start if guarded allocation itself fails. See [SECURITY.md](../SECURITY.md#guarded-memory-linux-only) for the full mechanism and its **Linux-only** scope - on macOS/Windows this bullet does not apply; see [Platform support](#platform-support) below.

What it doesn't defend against:

- **Compromised running daemon, request in flight** - while a request is actively using a persona's identity key or a bucket key (i.e. inside the brief guarded-memory-unlocked window, or in the ordinary heap for object-body plaintext), a root/kernel-level live memory read recovers that request's plaintext and the wrapping key that sealed it. Guarded memory closes idle-key disclosure (see above), not a live read timed to a request in flight. Session TTL (`[auth] max_ttl_seconds`) bounds the window a leaked identity key stays valid at all, independent of memory protection.
