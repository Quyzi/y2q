...
use crate::model::{ListBucketsResponse, ListOptions, ListPage, SearchOptions};
...
impl Y2qClient {
    ...
    /// Search objects by label `query` (`GET /api/v1/search`). The daemon parses
    /// and validates the query; a malformed query surfaces as a `400`
    /// [`ClientError`].
    pub async fn search_labels(
        ...
        let mut req = self.authed(self.inner.get(url)).query(&[("q", query)]);

        if let Some(ref b) = options.bucket {
            req = req.query(&[("bucket", b.as_str())]);
        }
        if let Some(ref p) = options.prefix {
            req = req.query(&[("prefix", p.as_str())]);
        }
