...
use y2q_core::crypto::{Argon2Params, UserRecord, UserStore, kdf};
...
impl AuthState {
    ...
    /// usernames: every one of its `CREDENTIAL_SLOTS` slots is a decoy, so a
    /// login attempt against it pays for exactly one Argon2 derivation plus
    /// four failing AEAD opens — the same cost profile as a real account
    /// rejecting a wrong password. Computed once at startup.
    fn build_dummy_record(argon2_config: &Argon2Config) -> UserRecord {
        ...
        let slots = kdf::decoy_slots_from("", 0, &params)
            .expect("build decoy slots for login-timing record");
        UserRecord {
            username: String::new(),
            created_at: 0,
            last_login: None,
...
mod tests {
    ...
    #[test]
    fn dummy_record_does_real_argon2_and_aead_work() {
        // Cheap costs keep the test fast; the point is that the dummy record
        // is genuinely wrapped material so its unwrap exercises Argon2id plus
        // four AEAD opens — the same work a real wrong-password login pays.
        ...
        // Every slot is a decoy: no password opens any of them.
        let kek = rec.kdf.derive_kek(b"whatever").unwrap();
        for (i, slot) in rec.slots.iter().enumerate() {
            let aad = kdf::slot_wrap_aad(&rec.username, i);
            assert!(kdf::unwrap_slot(&slot.wrapped, &kek, &aad).is_err());
        }
    }
}
