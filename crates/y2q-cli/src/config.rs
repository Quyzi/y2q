...
pub fn default_tokens_path() -> Result<PathBuf, CliError> {
...
impl CliConfig {
    ...
    pub fn save(&self, path: &Path) -> Result<(), CliError> {
        ...
        let text = toml::to_string_pretty(self).map_err(|e| CliError::Config(e.to_string()))?;
        atomic_write(path, text.as_bytes())?;
        set_mode_600(path);
        Ok(())
    }

    pub fn get_profile(&self, alias: &str) -> Result<&Profile, CliError> {
        self.profiles
            .get(alias)
            .ok_or_else(|| CliError::UnknownAlias(alias.to_owned()))
    }
...
#[cfg(unix)]
pub(crate) fn set_mode_600(path: &Path) {
    use std::os::unix::fs::PermissionsExt;
    let _ = std::fs::set_permissions(path, std::fs::Permissions::from_mode(0o600));
}

#[cfg(not(unix))]
pub(crate) fn set_mode_600(_path: &Path) {}
