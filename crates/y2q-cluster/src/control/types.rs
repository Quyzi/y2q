...
pub enum ControlCmd {
    ...
    ClaimBucketOwner {
        ...
        owner: String,
        /// The bucket's epoch-0 key material, pre-generated by the proposing
        /// node before this command is raft-proposed. Key generation
        /// involves randomness (a fresh ML-KEM-768 keypair and bucket wrap
        /// key), so it must happen exactly once and replicate as data —
        /// never be regenerated independently by each node applying the
        /// log, which would produce a different keypair per replica.
        /// Applied only when this claim actually wins (`cfg.owner` was
        /// `None`); a losing claim's pre-generated key material is simply
        /// discarded.
        key: BucketKeyVersion,
    },
...
impl ControlState {
    ...
    pub fn apply(&mut self, cmd: &ControlCmd) -> ControlResp {
        match cmd {
            ...
            ControlCmd::ClaimBucketOwner { bucket, owner, key } => {
...
mod tests {
    ...
    fn bucket_registry_apply() {
        ...
        // The winning claim's key material stuck; the losing claim's was discarded.
        assert_eq!(s.buckets["docs"].keys.len(), 1);
    ...
    fn user_record(name: &str, role: Role) -> UserRecord {
        ...
        let slots = y2q_core::crypto::kdf::decoy_slots_from(name, 0, &params).unwrap();
        UserRecord {
